function main () {
	...
	world = new World(global, canvas, [GLOBAL.AGENT_COLOR, 20],
																		[GLOBAL.AGENT_COLOR, 20]);
	...
	global.PAUSED = false;
	world = new World(global, canvas, [GLOBAL.AGENT_COLOR, null],
																		[GLOBAL.AGENT_COLOR, null]);
	world.externalInit = function () {
		this.stage.removeChild(this.bg); // hide the background
		this.stage.removeChild(this.info); // hide the info bar
	...
	global.INIT_AGENTS_VARIATIONS[1] = 20;
	global.FATHER_MUTATION_PROBS[1] = 0;
	global.MOTHER_MUTATION_PROBS[1] = 0;
	// autoplay
	global.AUTOPLAY = true;
	global.PAUSED = false;
	world = new World(global, canvas, [GLOBAL.AGENT_COLOR, null],
																		[GLOBAL.AGENT_COLOR, 22]);
	world.externalInit = function () {
		this.stage.removeChild(this.bg); // hide the background
		this.stage.removeChild(this.info); // hide the info bar
	...
	world.externalTick = function () {
		if (!createjs.Tween.hasActiveTweens(this.GLOBAL) &&
				(this.agents.filter(function (x) { return x.scaleX > 0.95 && x.scaleX < 1}).length >= 1 ||
	...
	global.WORLD_OFFSET_Y = 0; // no info bar, so take up the whole canvas
	global.DEATH_THRESHHOLD = Infinity; // can't die!
	global.INIT_AGENTS_VARIATIONS[1] = 0;
	global.FATHER_MUTATION_PROBS[1] = 1;
	global.MOTHER_MUTATION_PROBS[1] = 0;
	global.MUTATION_RATES[1] = 20;
	// autoplay
	global.AUTOPLAY = true;
	global.PAUSED = false;
	world = new World(global, canvas, [GLOBAL.AGENT_COLOR, null],
	...
	global = globalClone();
	global.OBSERVER_PERIOD = Infinity; // no predator period
	global.INITIAL_AGENT_OFFSETS[1] = 5;
	global.NUM_AGENTS = 4;
	world = new World(global, canvas, [GLOBAL.AGENT_COLOR, null],
																		['relative', 'relative']);
	world.externalInit = function () {
	...
	global.AUTOPLAY = true;
	global.PAUSED = false;
	world = new World(global, canvas, [GLOBAL.AGENT_COLOR, size],
																		['relative', 'relative']);
	world.externalInit = function () {
		//this.stage.removeChild(this.bg); // hide the background
	...
	global.NUM_AGENTS = 20;
	global.WORLD_OFFSET_Y = 0; // no info bar
	global.INITIAL_AGENT_OFFSETS[1] = 0; // v. hidden critters
	global.MODE_SWITCH_SPEED = 200; // fast mode switch
	// autoplay
	global.AUTOPLAY = true;
	global.PAUSED = false;
