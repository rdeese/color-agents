...
// NOT OK IN GLOBAL SCOPE
// WE CAN IGNORE THESE FOR NOW B/C WE'RE NOT USING AUTOPRED
var lastAutoKill; // appears in Info.js, should be property of info if we even
									// still need it.
...
function tick (event) {
	...
	if (!event.paused) {
		...
		if (event.WILL_DRAW) { GLOBAL.AGENTS_DIRTY = false; }
...
function main () {
	...
	stage.on("worldClick", function (e) {
		console.log("world click");
		info.handleWorldClick(e.mouseEvent, e.onAgent, e.agent);
	});
